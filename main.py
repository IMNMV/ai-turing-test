...
STUDY_MODE = os.getenv("STUDY_MODE", "HUMAN_WITNESS")  # Options: "AI_WITNESS" or "HUMAN_WITNESS"
# ---------------------------------
...
Length and form.                                                                                   
  ...
  occasionally a longer one. Most replies are short-to-medium (~5-15 words), with a good number very short. Only occasionally — roughly 1 in 5 — go long (20-30, messy and run-on). Don't make long the default.
  ...
  - People will sometimes ask if you're an AI, it's not an insane question so don't act too surprised. 
...
async def generate_ai_response(model, prompt:str, technique:Optional[str], user_profile:Dict, conversation_history:List[Dict], chosen_persona_key: str, social_style: str = "DIRECT", current_tactic_analysis: str = None, previous_researcher_notes: List[Dict] = None, time_remaining_display: str = None):
    ...
    if chosen_persona_key == "custom_extrovert":
        ...
        if social_style == "BLAND":
            bland_length_block = (
                "Mostly 1-6 words, often just 2-4. Sometimes a single blunt word "
                "(\"fine\", \"idk\", \"no clue\", \"yeah\"). Once in a while a slightly longer line "
                "(~10-12 words). Never settle into a uniform medium length, never a long rambling message."
            )
...
{f"The conversation timer on your screen currently shows {time_remaining_display} remaining. Only mention this if asked." if time_remaining_display else ""}
...
def assign_social_style_counterbalanced(db_session: Session) -> str:
    ...
    Filters by current STUDY_MODE so each condition balances independently.
    ...
    # Filter by study_mode so AI and human conditions balance independently
    ...
    print(f"📊 Counterbalanced style assignment [{STUDY_MODE}]: {count_map} → chose {chosen} (min={min_count}, ties={len(least_used)})")
    return chosen
...
async def initialize_study(data: InitializeRequest, db_session: Session = Depends(get_db)):
    ...
    if existing_session and existing_session.session_status == "pre_consent":
        ...
        try:
            ...
            existing_session.study_mode = STUDY_MODE  # Ensure study_mode is set
            existing_session.last_updated = datetime.utcnow()
